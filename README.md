...
読み方は「キューエルテス」くらいでしょうか。「クルテス」でもよいかな。

## 何が新しくなったか
　一番の違いは、OS非依存にしたことで、Linux / macOS / Windows（PowerShell）すべてで同じ動作になるはずです。(私はMacを持っていないためにMacでは未確認ですが、不具合があればご連絡ください)
  
 次に、データ（*lc.fitsファイル）を保存するディレクトリが、カレントディレクトリ（実行したディレクトリなので不定）から~/tess_data/（固定）になりました。これにより、どのディレクトリで実行してもデータが保存される場所は１箇所になり集中管理できるようになりました。ちなみに、Windowsでは、C:\Users\ユーザー名\tess_data\のフォルダーになります。ディレクトリは自動で作られます。別の場所にフォルダーを置きたい場合は、以下の「オプションの使い方」の--data-dir　の説明を読んで下さい。また、光度曲線を閲覧したデータについては、テキストデータも自動で保存するようにしました。ファイル名は同名で拡張子がfitsからtxtに置き換えたものです。
 
 あと、これまでは観測期間全体を俯瞰する光度曲線の一覧表示を見たい時は -fオプションで起動していましたが、新しくデフォルト設定のスキャンモード閲覧中からでもすぐに切り替えられるようにしました（一時停止にしてfキーを押せばよい）。逆に -f オプションから起動しても mキーを押せば、スキャンモードに移行します。

 最後に、後述のように以前は独立したコードのplot_lcfits.pyは廃止し、qltess.pyに統合しました。オプションで利用できます。
