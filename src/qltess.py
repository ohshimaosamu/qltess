...
def download_tess_lc_for_tic(tic_id, download_root=None, prefer_authors=None):
    """
    TIC番号に対応する TESS light curve FITS を lightkurve 経由で取得する。
    SPOC / TESS-SPOC / QLP を順に試す。
    保存先: <download_root>/TICxxxx/
    """
...
def find_lc_dirs(tic_id, root=None):
    """
    <root>/TICxxxx/ 以下を再帰探索して *_lc.fits を拾う。
    HLSP, QLP, SPOC などをまとめて拾う。
    """
    if root is None:
        root = get_default_data_dir()

    base_dir = Path(root).expanduser() / f"TIC{tic_id}"
    if not base_dir.is_dir():
        return base_dir, []

    found = []
    for pat in ("*_lc.fits", "*lc.fits"):
    ...
    for fp in fits_list:
        ...
        parent = rel.parent
        display_name = str(parent) if str(parent) != "." else fp.name
...
def read_lightcurve(fits_path):
    with fits.open(fits_path) as hdul:
        ...
        bjd_base = int(np.floor(np.min(x)))

        return x, y, ylabel, bjd_base
...
def plot_lightcurve_full(fits_path, display_name):
    """
...
class LCScanner:
    def __init__(
        ...
        self.ax.set_xlabel(f"BJD - {self.bjd_base}")
    ...
    def save_current_window(self):
        ...
        fname = (
            ...
            f"__BJDm{self.bjd_base}_{int(round(left))}_{int(round(right))}.png"
        )
...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default=None,
        help="データ保存ディレクトリ (default: ~/tess_data)"
    )
    ...
    try:
        if is_tic_like(target_input):
            ...
            tic_id = resolve_name_to_tic(target_input)
    ...
    data_root = Path(args.data_dir).expanduser() if args.data_dir else get_default_data_dir()
